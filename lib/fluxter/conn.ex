defmodule Fluxter.Conn do
  ...
  def write(worker, name, tags, fields, timestamp, timestamp_unit)
      when (is_binary(name) or is_list(name)) and is_list(tags) and is_list(fields) do
    GenServer.cast(worker, {:write, name, tags, fields, timestamp, timestamp_unit})
  end
  ...
  def handle_cast({:write, name, tags, fields, timestamp, timestamp_unit}, conn) do
    packet = Packet.build(conn.header, name, tags, fields, timestamp, timestamp_unit)
